program NS
	...
	if(modus == 0) then
		...
		open(6,file = 'Z_Klassisch_3D.txt', status = 'old')
		open(7,file = 'oZ_Klassisch_3D.txt', status = 'old')
		open(8,file = 'i_Klassisch_3D.txt', status = 'old')
		open(9,file = 'v_Klassisch_3D.txt',status = 'old')
		open(10,file = 'E_Klassisch_3D.txt',status = 'old')
		
		do j = 1,40
		
			T = real( 10 * j, 16)
			call sampler(500, d, T, v_max, Z, oZ, i, E, v)
		...
		open(6,file = 'Z_100K_Klassisch_3D.txt', status = 'new')
		open(7,file = 'oZ_100K_Klassisch_3D.txt', status = 'new')
		open(8,file = 'i_100K_Klassisch_3D.txt', status = 'new')
		
		open(9,file = 'Z_200K_Klassisch_3D.txt', status = 'new')
		open(10,file = 'oZ_200K_Klassisch_3D.txt', status = 'new')
		open(11,file = 'i_200K_Klassisch_3D.txt', status = 'new')
		
		open(12,file = 'Z_300K_Klassisch_3D.txt', status = 'new')
		open(13,file = 'oZ_300K_Klassisch_3D.txt', status = 'new')
		open(14,file = 'i_300K_Klassisch_3D.txt', status = 'new')
		
		open(15,file = 'Z_400K_Klassisch_3D.txt', status = 'new')
		open(16,file = 'oZ_400K_Klassisch_3D.txt', status = 'new')
		open(17,file = 'i_400K_Klassisch_3D.txt', status = 'new')
	
		do j = 500,10000,500
		
			T = real( 100, 16)
			call sampler(j, d, T, v_max, Z, oZ, i, E, v)
...
subroutine prior(sample, d, v_max) !Creates a d-dimensional Sample from the Prior
	...
	do l = 1,d
		...
		sample(l) = real(2.0,16) * v_max * u - v_max
...
real (kind = 16) function lnL(theta, d, T, v_max) !ln(likelihood)
	...
	integer :: d
	real (kind = 16) :: T, v_max, pi, m, kB, v2
	...
	v2 = theta(1)**2 + theta(2)**2 + theta(3)**2
	
	lnL = real(3.0,16) * real( log( 2.0 * real(v_max) ) , 16) - ( (m * v2) / ( real(2.0,16) * kB * T ) )
...
subroutine sampler(K, d, T, v_max, Z, oZ_H, i, E, v)
	...
	open(1, file = 'Results_Klassisch_3D.txt', status = 'old')
	open(2, file = 'Weights_Klassisch_3D.txt', status = 'old')
	open(3, file = 'Deadpoints_Klassisch_3D.txt', status = 'old')
	open(4, file = 'lnL_Klassisch_3D.txt', status = 'old')
	open(5, file = 'PriorVolume_Klassisch_3D.txt', status = 'old')
	...
	do while( halt == 0 )
		...
		v = v + w * real( sqrt(real( livepoints(j_min,1) ** 2 + livepoints(j_min,2) ** 2 + livepoints(j_min,3) ** 2 )) ,16)
		
		E = E +  w * real(0.5,16) * m * ( livepoints(j_min,1) ** 2 + livepoints(j_min,2) ** 2 + livepoints(j_min,3) ** 2 )
		...
		!If directly sampling from the prior does take too long
			...
			do while( test2 == 0 )
				...
				do l = 1,d
					...
					sigma_max(l) = v_max * ( X_i ** ( real(1.0,16) / real(d,16) ) )
